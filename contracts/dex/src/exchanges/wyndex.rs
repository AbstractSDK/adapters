...
use wyndex::pair::{PoolResponse, SimulationResponse};
...
impl DEX for WynDex {
    ...
    fn provide_liquidity(
        ...
        // execute msg
        let msg = wyndex::pair::ExecuteMsg::ProvideLiquidity {
    ...
    fn withdraw_liquidity(
        ...
        let pair_address = pool_id.expect_contract()?;
        let hook_msg = wyndex::pair::Cw20HookMsg::WithdrawLiquidity { assets: vec![] };
...
fn cw_asset_to_wyndex_valid(asset: &Asset) -> Result<wyndex::asset::AssetValidated, DexError> {
    match &asset.info {
        AssetInfoBase::Native(denom) => Ok(wyndex::asset::AssetValidated {
            amount: asset.amount,
            info: wyndex::asset::AssetInfoValidated::Native(denom.clone()),
        }),
        AssetInfoBase::Cw20(contract_addr) => Ok(wyndex::asset::AssetValidated {
            amount: asset.amount,
